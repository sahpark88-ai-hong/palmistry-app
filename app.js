...
function buildDetailedReport(scores, quality, interpretation, advice) {
  ...
  return {
    summary: `${confidenceText} 강점은 ${dominantLine[0]} 흐름에서 가장 잘 보입니다. 다만 이 강점에만 기대면 다른 선택지를 놓칠 수 있어, 결과는 자기성찰용 힌트로 가볍게 보는 편이 좋습니다.`,
    current: `${interpretation.right} 장점은 실행 방향을 잡기 쉽다는 점이고, 주의점은 확신이 늦어질 때 판단을 미루기 쉽다는 점입니다. 작은 순서부터 정리하면 흐름이 안정됩니다.`,
    emotion: `${interpretation.emotion} ${interpretation.relation} 관계에서는 진심이 강점이지만, 표현 속도가 맞지 않으면 오해가 생길 수 있어 확인 대화가 도움이 됩니다.`,
    advice: `${advice.text} 오늘은 장점을 크게 쓰되, 성급한 결론보다 한 번 더 확인하는 태도가 좋습니다. 점수는 운명 판단이 아니라 사진에서 감지된 선명도와 후보선 강도를 바탕으로 한 참고 지표입니다.`,
  };
}
...
function buildPerspicaciousReport(scores, interpretation) {
  ...
  return {
    ideal:
      heartHigh && headHigh
        ? "감정적으로 따뜻하면서도 대화의 결이 잘 맞는 사람에게 끌리는 타입입니다. 다만 기준이 높아지면 상대를 오래 관찰만 할 수 있으니, 좋은 신호가 보이면 작은 표현부터 시작하는 것이 좋습니다."
        : heartHigh
          ? "정서적으로 안정감을 주고 애정 표현이 자연스러운 사람에게 끌릴 가능성이 큽니다. 장점은 진심을 잘 느낀다는 점이고, 주의점은 분위기에 기대어 현실 조건을 늦게 볼 수 있다는 점입니다."
          : "처음부터 강하게 몰입하기보다 편안하게 거리를 좁혀 가는 사람에게 호감을 느끼는 흐름입니다. 신중함은 장점이지만 너무 오래 거리를 두면 기회가 흐려질 수 있습니다.",
    romance:
      fateHigh
        ? "연애에서 방향이 정해지면 책임감 있게 관계를 이끌어 가려는 편입니다. 다만 스스로 확신이 생기기 전까지는 관찰 시간이 길어질 수 있어, 감정과 계획을 함께 말해주는 것이 좋습니다."
        : "빠른 결정보다는 분위기와 상호 반응을 보며 천천히 깊어지는 쪽에 가깝습니다. 섬세함은 장점이지만 작은 변화에 과하게 의미를 부여하지 않도록 균형이 필요합니다.",
    affection:
      lifeHigh
        ? "애정 표현은 생활 속에서 챙겨주고 함께 시간을 쌓는 방식으로 드러나기 쉽습니다. 행동으로 안정감을 주는 점은 강점이지만, 말로 확인해 주지 않으면 상대가 부족하게 느낄 수 있습니다."
        : "애정은 깊지만 표현 속도는 신중한 편입니다. 오래 지키려는 힘은 장점이지만, 마음을 너무 늦게 보여주면 상대가 거리감으로 받아들일 수 있습니다.",
    intimacy:
      headHigh
        ? "친밀감에서는 감정의 속도보다 신뢰와 심리적 안정감을 먼저 확인하려는 경향이 있습니다. 배려와 대화는 강점이지만, 지나친 검토는 자연스러운 흐름을 막을 수 있어 편안함도 함께 보는 것이 좋습니다."
        : "친밀감은 분위기와 감정 교류의 영향을 많이 받는 흐름입니다. 자연스럽게 깊어지는 장점이 있지만, 순간의 감정에만 기대지 않도록 서로의 속도와 경계를 확인하는 것이 좋습니다.",
  };
}
